...
class EnvironmentSEWDQNExtended(Environment, Serializable):
    ...
    def __init__(self, system=None, system_file=None,
        ...
        for weight in weights:
            for tmax in tmaxs:
                i+=1
                env_single = self._init_single_env(weight,tmax,i,initial_sol,initial_sol_file,system_file,weight_id,len(weights))
    ...
    def step(self, _action):
            ...
            if self._initial:
                ...
                if self._envs[self.chosen_env]._current_step >= self._max_env_step:
                    if not self._round_robin:
                        ...
                        if self.chosen_env == len(self._envs):
                            self.chosen_env = 0
                            while self._env[self.chosen_env]._current_step >= self._max_env_step:
                                self.chosen_env += 1
        ...
        absorbing = False#self._state[2] < self._battery_min_percentage
    ...
    def convert_to_original_state_with_configs(self, state):
        ...
        threshold = state[8] * (self._elements_max[8] - self._elements_min[8]) + self._elements_min[8]

        return [r_wifi,r_5G,l_SEW,l_phone,l_cloud,weight1,weight2,weight3,threshold]
    ...
    def normalize_state_with_all_configs(self, state):
        ...
        weight2 = state[6]
        weight3 = state[7]
        threshold_norm = (state[8] - self._elements_min[8]) / (self._elements_max[8] - self._elements_min[8])

        return [r_wifi_norm,r_5G_norm,l_SEW_norm,l_phone_norm,l_cloud_norm,weight1,weight2,weight3,threshold_norm]


    def _init_single_env(self,weight,tmax,env_number,initial_sol,initial_sol_file,system_file,weight_id,len_weights):
        ...
        if len_weights>1:
            idx = weight_id/(len_weights-1)
            if idx == 0:
                env_single._initial_state.append(1)
                env_single._initial_state.append(0)
                env_single._initial_state.append(0)
            if idx == 0.5:
                env_single._initial_state.append(0)
                env_single._initial_state.append(1)
            ...
            if idx == 1:
                ...
                env_single._initial_state.append(0)
                env_single._initial_state.append(1)
            env_single.weight_state = weight_id/(len_weights-1)
        else:
            env_single._initial_state.append(1)
            env_single._initial_state.append(0)
            env_single._initial_state.append(0)
            env_single.weight_state = 0
    ...
    def _step_in_env(self, env_, chosen_action):
        ...
        env._previous_state = [r_WIFI,r_5G,l_SEW,l_phone,l_cloud,weight1,weight2,weight3,threshold]
