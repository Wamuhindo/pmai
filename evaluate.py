...
def get_core_params(env_params_files, system_files, initial_configs):
    ...
    for (env_params_file, system_file, initial_config) in zip(env_params_files, system_files, initial_configs):
        ...
        data["NetworkDomains"]["WIFI7"]["trace"]="traces/validation/WIFI7_validation.csv"
        data["NetworkDomains"]["5G"]["trace"]="traces/validation/5G_validation.csv"
        ...
        directory = os.path.dirname(os.path.abspath(env_params_file)) + "/logs_valid/{}".format(currentTime)
                    ...
                    for scenario in names_scenario:
                        ...
                        for a in Methods:
                            ...
                            for i in range(n_experiment):
                                dir_model = ""
                                for root,dirs,files in os.walk(os.path.dirname(os.path.abspath(env_params_file)) + "/logs/"):
                                    if "exp_{}_".format(i+1) in root:
                                        dir_model = root
                                        break
                                if dir_model =="":
                                    print("le dossier contenant le model n'existe pas")
                                    sys.exit(1)

                                core_params.append((a, e, i+1, variation, scenario, lr, lr_exp, weights,
                                                    t_max, step_per_checkpoint, N_steps, DQNParameters,
                                                    input_file_names,state_discretization, directory4,extended_state,steps_per_env,all_steps_per_env,round_robin_policy,evaluation,dir_model))
                else:
...
def experiment(core_param):
    ...
    dir_model = core_param[20]
    file_name = core_param[21]
    ...
    filename = Dir + "/validation_{}.txt".format(n_steps)
    setup = {}
        ...
        if DQNParameters:
            if not extended_state:
                mdp = EnvironmentSEWDQN(system_file=input_file_names[0],
                                        ...
                                        logger=Logger1(stream=file_stream,verbose=3), variations=variation,max_env_step=n_steps)
            else:
            ...
            # DQN Parameters
            try:
                n_steps_per_fit = 1#DQNParameters["n_steps_per_fit"]
                initial_replay_size = 0#DQNParameters["initial_replay_size"]
                max_replay_size = 0#DQNParameters["max_replay_size"]
                target_update_frequency = n_steps#DQNParameters["target_update_frequency"]  # every (n_steps_per_fit * target_update_frequency) steps, the target network weights will be updated (fit function in abstract_dqn.py)
                batch_size = 1#DQNParameters["batch_size"]
    ...
    with open(setup_json, "w") as f:
        f.write(system)
    weights = [{"w_energy_SEW":0.03,"w_energy_Phone":0.02, "w_exec":0.93, "w_config":0.02, "w_conn":0.00},
    ...
    def callback_step(step):
        ...
        mdp._save_info_to_file(
            mdp._get_info(), {"latency": mdp._latency,"energy":mdp._energy, "max_diff_Qs": max_diff_Qs,
    ...
    if evaluation:
        ...
        pickle_file = "{}/list_params_{}.pickle".format(dir_model,n_steps)
        list_params = []
        with open(pickle_file, "rb") as fp:  # Unpickling
            list_params = pickle.load(fp)
        
        zip_file = "{}/agent_{}.zip".format(dir_model,n_steps)
...
if __name__ == '__main__':
    ...
    if args.Apps_dir:
        ...
        env_params_file = ["env_config_old.json"]
    ...
    if not start_from_checkpoint:
        for (env_p_file, sys_file, init_file) in zip(env_params_file,system_file,initial_config):
            core_params = get_core_params([env_p_file], [sys_file], [init_file])
            #experiment(core_params[0])
            with parallel_backend('multiprocessing', n_jobs=-1):
                out = Parallel()(delayed(experiment)(core_param+(args.logfile,)) for core_param in core_params)
            output.extend(out)
    else:
    ...
    for out in all_outs:
        ...
        plt.legend()
        fig.savefig(directory4 + "/" + 'valid.png')
